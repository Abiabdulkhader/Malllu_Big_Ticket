...
export default function App() {
  ...
  const handleWhatsAppShare = () => {
    ...
    // Using bold formatting and clean text instead of emojis to prevent encoding glitches
    const message = `*കോടീശ്വരൻ പ്ലാൻ: ${currentMonthName} ${currentYear}*\n\n` +
    ...
    // Switched to api.whatsapp.com which is more reliable for long text strings
      ...
      <div className="w-full sm:max-w-md mx-auto bg-slate-50 sm:bg-white sm:shadow-2xl sm:shadow-emerald-900/5 sm:border border-slate-100 sm:rounded-[2.5rem] overflow-hidden relative min-h-screen sm:min-h-0">
          ...
          <div className="pb-28">
              ...
              <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-3xl p-6 text-white shadow-xl shadow-emerald-900/20 relative overflow-hidden border border-emerald-500/30">
                ...
                <div className="relative z-10 text-center mb-6">
                  <div className="text-5xl font-black mb-1 flex items-baseline justify-center gap-1 drop-shadow-lg">
                    {totalCollected} <span className="text-xl font-medium text-emerald-200">/ 500</span>
                  </div>
                  <p className="text-emerald-100 text-sm font-bold uppercase tracking-wider">AED കിട്ടി</p>
                </div>

                <div className="relative z-10">
                  <div className="flex justify-between text-xs font-semibold mb-2 text-emerald-50 drop-shadow-sm">
                    <span>{paidCount === 1 ? 'ഒരാൾ കാശ് തന്നു' : `${paidCount} പേർ കാശ് തന്നു`}</span>
                    <span>{unpaidCount === 1 ? 'ഒരുത്തൻ മുങ്ങി നടക്കുന്നു' : `${unpaidCount} പേർ മുങ്ങി നടക്കുന്നു`}</span>
                  </div>
                ...
                {isGoalReached && (
                  <div className="mt-5 bg-emerald-500/30 backdrop-blur-md border border-emerald-400/50 rounded-2xl p-3 flex items-center justify-center gap-2 animate-pulse text-amber-300 font-black text-sm shadow-lg">
                    <Ticket className="w-5 h-5" /> കാശ് സെറ്റ്! പോയി ടിക്കറ്റ് എടുക്കെടാ! 🏃‍♂️
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="px-6 mb-6 space-y-3">
              <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex items-center justify-between gap-4 transition-all hover:shadow-md hover:border-emerald-100">
                <div className="flex items-center gap-3 text-slate-700 font-medium">
                  ...
                  <div>
                    <p className="text-sm font-extrabold text-slate-800">ഇത്തവണ ടിക്കറ്റ് എടുക്കുന്നത് 🎯</p>
                  </div>
                </div>
                <select
                  disabled={!isAdmin}
            ...
            <div className="px-6 mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-bold text-slate-400 bg-slate-200/50 px-2 py-1 rounded-lg">{paidCount}/10 പേർ</span>
              </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-black transition-all duration-300 ${
                            isPaid ? 'bg-gradient-to-br from-emerald-400 to-emerald-600 text-white shadow-lg shadow-emerald-500/30' : 'bg-slate-100 text-slate-400'
                          }`}>
        ...
        <div className="fixed sm:absolute bottom-0 w-full sm:max-w-md bg-white/80 backdrop-blur-xl border-t border-slate-100 flex justify-around p-3 pb-safe shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] z-50">
          <button 
            ...
            className={`flex flex-col items-center p-2 rounded-2xl w-24 transition-all duration-200 ${activeView === 'dashboard' ? 'text-emerald-600 bg-emerald-50 scale-105' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center p-2 rounded-2xl w-24 transition-all duration-200 ${activeView === 'history' ? 'text-emerald-600 bg-emerald-50 scale-105' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'}`}
          >
